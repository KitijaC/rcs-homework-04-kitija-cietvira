<?php
    ...
    if ($stmt->prepare($sql)) {
        ...
        if ($stmt->execute()) {
            ...
            if ($stmt->num_rows == 1) {
                ...
                if ($stmt->fetch()) {
                    $signedInUserId = $_SESSION["id"];

                    if ((int)$signedInUserId !== (int)$postOwner_user_id) {
                        header("location: ../view/profile.php?user-id=$signedInUserId"); 
                        $stmt->close();
                        exit;
                    }
                }
    ...
    $sql = "UPDATE posts SET post_deleted = 1 WHERE id = ?";
